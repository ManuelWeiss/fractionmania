...
export function ComparisonExercise() {
  ...
  type ComparisonQuestion = ReturnType<typeof parseComparisonLine>
  ...
  const handleNext = useCallback(() => {
    setAnswers((arr) => [
      ...arr,
      { correct: isCorrect === true, attempts: attempts + (isCorrect ? 0 : 1) },
    ])
    ...
    >
      <div className="space-y-8">
        ...
        <div className="mb-4">
          <div className="flex justify-between items-center mb-1">
            <span className="text-sm font-medium text-gray-700">
              Problem {current + 1} of {questions.length}
            </span>
        ...
        <div className="flex items-center justify-center space-x-8 text-4xl font-bold">
          <div className="text-center">{renderMixedFraction(left)}</div>
          <div className="flex space-x-4">
            <button
          ...
          <div className="text-center">{renderMixedFraction(right)}</div>
        </div>
