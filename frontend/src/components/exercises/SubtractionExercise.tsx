...
export function SubtractionExercise() {
  ...
  const handleSubmit = useCallback(() => {
    ...
    const diffNum = q.num1 * (commonDen / q.den1) - q.num2 * (commonDen / q.den2);
    const { num: simplestNum, den: simplestDen } = simplify(diffNum, commonDen);
  ...
  if (current >= TOTAL_QUESTIONS || completed) {
    ...
    return (
      <BaseExercise
        ...
        description="Subtract the two fractions and enter the answer in simplest form."
        ...
        <div className="mb-4 flex items-center justify-center text-3xl font-bold space-x-4">
          {React.createElement(
            ...
            [
              ...
              React.createElement('mo', { key: 'minus' }, '-'),
              React.createElement(
                'mfrac',
                { key: 'right' },
