...
import { simplify, generateTwoFractions } from './exerciseUtils';
...
export function MultiplicationExercise() {
  ...
  const [questions, setQuestions] = useState(() => Array.from({ length: TOTAL_QUESTIONS }, generateTwoFractions));
  ...
  const handleSubmit = useCallback(() => {
    ...
    const prodNum = q.num1 * q.num2;
    const prodDen = q.den1 * q.den2;
    const { num: simplestNum, den: simplestDen } = simplify(prodNum, prodDen);
    const whole = parseInt(userWhole) || 0;
    const num = parseInt(userNum) || 0;
    const den = parseInt(userDen) || 1;
    const improperNum = whole * den + num;
    const correct =
      improperNum === simplestNum &&
      den === simplestDen;
  ...
  if (current >= TOTAL_QUESTIONS || completed) {
    ...
    return (
      <BaseExercise
        ...
        description="Multiply the two fractions and enter the answer in simplest form."
        ...
        <div className="mb-4 flex items-center justify-center text-3xl font-bold space-x-4">
          {React.createElement(
            ...
            [
              ...
              React.createElement('mo', { key: 'times' }, '×'),
              React.createElement(
                'mfrac',
                { key: 'right' },
