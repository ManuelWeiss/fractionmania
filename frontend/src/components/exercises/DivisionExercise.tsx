...
export function DivisionExercise() {
  ...
  const handleSubmit = useCallback(() => {
    ...
    const divNum = q.num1 * q.den2;
    const divDen = q.den1 * q.num2;
    const { num: simplestNum, den: simplestDen } = simplify(divNum, divDen);
  ...
  if (current >= TOTAL_QUESTIONS || completed) {
    ...
    return (
      <BaseExercise
        ...
        description="Divide the two fractions and enter the answer in simplest form."
        ...
        <div className="mb-4 flex items-center justify-center text-3xl font-bold space-x-4">
          {React.createElement(
            ...
            [
              ...
              React.createElement('mo', { key: 'divide' }, '÷'),
              React.createElement(
                'mfrac',
                { key: 'right' },
