...
const FractionQuestion: React.FC<FractionQuestionProps> = ({ mode }) => {
  ...
  const [setCorrectAnswerRaw] = useState<Fraction | null>(null) // for hint
          ...
          {Math.abs(correctAnswer.num) >= correctAnswer.den && (
            <span style={{ marginLeft: 16, fontSize: '0.7em', color: '#333' }}>
              ...
              {(() => {
                ...
                return num === 0 ? whole : renderMixedNumberMathML(whole, num, den)
              })()}
              )
            </span>
          )}
  ...
  return (
    ...
    >
      ...
      >
        <h2 style={{ textAlign: 'center', fontWeight: 700, color: '#6366f1', letterSpacing: 1 }}>
          Practicing {mode} Fractions
        </h2>
